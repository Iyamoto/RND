...
def exponential_moving_average(df, n):
    ...
    :return: pandas.DataFrame
    """
    EMA = pd.Series(df['4. close'].ewm(span=n, min_periods=n).mean(), name='EMA')
    return EMA
...
def macd(df, n_fast=12, n_slow=26):
    ...
    :return: pandas.DataFrame
    """
    EMAfast = pd.Series(df['4. close'].ewm(span=n_fast, min_periods=n_slow).mean())
    EMAslow = pd.Series(df['4. close'].ewm(span=n_slow, min_periods=n_slow).mean())
    MACD = pd.Series(EMAfast - EMAslow, name='MACD')
    MACDsign = pd.Series(MACD.ewm(span=9, min_periods=9).mean(), name='MACD_Sign')
...
def relative_strength_index(df, n):
    ...
    DoI = [0]
    while i + 1 <= df.index[-1]:
        UpMove = df.loc[i + 1, '2. high'] - df.loc[i, '2. high']
        DoMove = df.loc[i, '3. low'] - df.loc[i + 1, '3. low']
        if UpMove > DoMove and UpMove > 0:
            UpD = UpMove
        else:
            UpD = 0
    ...
    RSI = pd.Series(100.0 * PosDI / (PosDI + NegDI), name='RSI_' + str(n))
    # df = df.join(RSI)
    return RSI
